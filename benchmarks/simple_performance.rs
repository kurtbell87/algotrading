...
use algotrading::market_data::{FastFeatureVector, FastMarketState, LAST_PRICE_IDX, VOLUME_IDX};
...
fn main() {
    ...
    for &num_events in &[100_000, 500_000, 1_000_000] {
        ...
        println!(
            "2. With cloning: {:.0} events/s ({:.1}x slower than raw)",
            clone_throughput,
            raw_throughput / clone_throughput
        );
        ...
        println!(
            "5. Combined optimized: {:.0} events/s ({:.1}% of 18M target)",
