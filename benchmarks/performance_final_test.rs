...
fn print_summary(read: f64, backtest: f64, direct: f64) {
    ...
    println!("\nThroughput (events/s):");
    println!(
        "  Raw reading:    {:.0} ({:.1}% of 18M)",
        read,
        (read / 18_000_000.0) * 100.0
    );
    println!(
        "  Backtest:       {:.0} ({:.1}% of 18M)",
        backtest,
        (backtest / 18_000_000.0) * 100.0
    );
    println!(
        "  Direct:         {:.0} ({:.1}% of 18M)",
        direct,
        (direct / 18_000_000.0) * 100.0
    );
    ...
    if backtest < 6_000_000.0 {
        println!(
            "  ❌ Backtest engine has severe overhead ({}x slowdown)",
    ...
    if direct > backtest * 2.0 {
        println!(
            "  ✅ Direct processing shows potential for {}x improvement",
