...
impl BenchmarkStrategy {
    fn new() -> Self {
        let config = StrategyConfig::new("Benchmark".to_string(), "Benchmark Strategy")
            .with_instrument(1);
...
impl Strategy for BenchmarkStrategy {
    ...
    fn on_market_event(&mut self, _event: &MarketEvent, _context: &StrategyContext) -> StrategyOutput {
        self.event_count += 1;
