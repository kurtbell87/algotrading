...
use algotrading::market_data::{FastMarketState, FastFeatureVector};
use algotrading::market_data::{LAST_PRICE_IDX, SPREAD_IDX, PRICE_RETURN_IDX, VOLUME_IDX, TRADE_COUNT_IDX};
                ...
                b.iter(|| {
                    ...
                    for event in &events {
                        ...
                        if let MarketUpdate::Trade(trade) = event {
                            features.update_from_trade(trade, market_state.get_last_price(trade.instrument_id));
                        }
    ...
    group.bench_function("optimized_pipeline", |b| {
        b.iter(|| {
            ...
            if throughput >= 15_000_000.0 {
                ...
                println!("✅ GOOD: Significant improvement ({:.1}x vs 3.1M baseline)", 
