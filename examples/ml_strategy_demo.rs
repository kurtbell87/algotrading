...
use algotrading::python_bridge::models::{PythonModelFactory, PythonModel};
...
fn load_ml_model() -> Result<Box<dyn PythonModel>, Box<dyn std::error::Error>> {
    ...
    println!("Loading ML model with {} features: {:?}", feature_names.len(), feature_names);
    
    // Load the scikit-learn model
    let model_path = "python_ml/trading_model.joblib";
    let model = PythonModelFactory::create_sklearn_model(
        model_path,
        feature_names,
        model_type,
    )?;
...
fn create_sample_market_data() -> Vec<MarketEvent> {
    ...
    for i in 0..1000 {
        ...
        let trade_event = TradeEvent {
            ...
            aggressor_side: if rand::random::<f64>() > 0.5 { Side::Bid } else { Side::Ask },
            timestamp,
            buyer_order_id: None,
...
fn run_ml_strategy_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match model.predict(&test_features) {
        Ok(prediction) => {
            println!("  Sample prediction: signal={:.3}, confidence={:.3}", 
...
fn main() {
    ...
    std::env::set_var("PYO3_PYTHON", "/Users/brandonbell/LOCAL_DEV/algotrading/python_ml/.venv/bin/python");
