...
fn process_verify_data(path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mapping in mappings {
        println!(
            ...
            mapping.raw_symbol, mapping.intervals[0].symbol
        );
    ...
    for inst_id in market.instruments() {
        ...
        match (bid, ask) {
            ...
            (Some(b), None) => {
                println!("Bid: {} @ {:.2} | Ask: None", b.size, b.price as f64 / 1e9)
            }
            (None, Some(a)) => {
                println!("Bid: None | Ask: {} @ {:.2}", a.size, a.price as f64 / 1e9)
            }
...
fn format_timestamp(ts_ns: u64) -> String {
    ...
    let dt = time::OffsetDateTime::from_unix_timestamp(seconds).expect("valid timestamp");
