...
use crate::market_data::events::{BBOUpdate, MarketEvent, SessionEvent, TradeEvent};
...
impl MarketStateManager {
    ...
    fn process_session_event(&mut self, event: &SessionEvent) {
        ...
        match event.event_type {
            SessionEventType::MarketOpen => {
                ...
                self.session_stats
                    .insert(event.instrument_id, SessionStatistics::default());
            }
    ...
    pub fn get_recent_trades(&self, instrument_id: InstrumentId, count: usize) -> Vec<RecentTrade> {
        self.recent_trades
            .get(&instrument_id)
            .map(|trades| trades.iter().rev().take(count).cloned().collect())
            .unwrap_or_default()
    }
    ...
    pub fn create_snapshot(&self, instrument_id: InstrumentId) -> Option<MarketState> {
        ...
        let mid_price = match (&best_bid, &best_ask) {
            (Some(bid), Some(ask)) => {
                ...
                Some(Price::from_f64(
                    (bid_price.as_f64() + ask_price.as_f64()) / 2.0,
                ))
            }
            _ => None,
        };
