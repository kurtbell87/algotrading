...
impl std::fmt::Display for PythonModelError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PythonModelError::InitializationError(msg) => write!(f, "Model initialization error: {}", msg),
...
pub trait PythonModel: Send + Sync {
    ...
    fn predict_batch(&self, features: &[PyFeatureVector]) -> Result<Vec<PyPrediction>, PythonModelError>;
...
impl SklearnModel {
    ...
    pub fn from_pickle<P: AsRef<Path>>(
        ...
        Python::with_gil(|py| {
            ...
            let pickle = py.import_bound("pickle")
                .map_err(|e| PythonModelError::InitializationError(format!("Failed to import pickle: {}", e)))?;
            
            // Load the model
            let file = std::fs::File::open(&model_path)
                .map_err(|e| PythonModelError::InitializationError(format!("Failed to open model file: {}", e)))?;
            
            let model_bytes = std::io::Read::bytes(file)
                .collect::<Result<Vec<_>, _>>()
                .map_err(|e| PythonModelError::InitializationError(format!("Failed to read model file: {}", e)))?;
            
            let model = pickle.call_method1("loads", (model_bytes,))
                .map_err(|e| PythonModelError::InitializationError(format!("Failed to unpickle model: {}", e)))?;
            
            // Verify model has predict method
            if !model.hasattr("predict")
                .map_err(|e| PythonModelError::InitializationError(format!("Error checking predict method: {}", e)))? {
                return Err(PythonModelError::InvalidModel("Model does not have predict method".to_string()));
            }
            
            let mut metadata = HashMap::new();
            metadata.insert("model_path".to_string(), model_path.as_ref().to_string_lossy().to_string());
    ...
    fn prepare_features(&self, features: &PyFeatureVector) -> Result<PyObject, PythonModelError> {
        Python::with_gil(|py| {
            let numpy = py.import_bound("numpy")
                .map_err(|e| PythonModelError::FeatureError(format!("Failed to import numpy: {}", e)))?;
            ...
            for feature_name in &self.feature_names {
                match features.get_feature(feature_name) {
                    ...
                    None => return Err(PythonModelError::FeatureError(
                        format!("Missing feature: {}", feature_name)
                    )),
                }
            }
            ...
            let np_array = numpy.call_method1("array", (&py_list,))
                .map_err(|e| PythonModelError::FeatureError(format!("Failed to create numpy array: {}", e)))?;
    ...
    fn prepare_features_batch(&self, features: &[PyFeatureVector]) -> Result<PyObject, PythonModelError> {
        Python::with_gil(|py| {
            let numpy = py.import_bound("numpy")
                .map_err(|e| PythonModelError::FeatureError(format!("Failed to import numpy: {}", e)))?;
...
impl PythonModel for SklearnModel {
    fn predict(&self, features: &PyFeatureVector) -> Result<PyPrediction, PythonModelError> {
        Python::with_gil(|py| {
            ...
            // Make prediction
            let prediction = model_ref.call_method1("predict", (&feature_array,))
                .map_err(|e| PythonModelError::PredictionError(format!("Prediction failed: {}", e)))?;
            
            // Extract prediction value (assume single output for now)
            let pred_array = prediction.downcast::<PyList>()
                .map_err(|e| PythonModelError::PredictionError(format!("Failed to extract prediction array: {}", e)))?;
            
            let signal: f64 = pred_array.get_item(0)
                .map_err(|e| PythonModelError::PredictionError(format!("Failed to get prediction value: {}", e)))?
                .extract()
                .map_err(|e| PythonModelError::PredictionError(format!("Failed to convert prediction to f64: {}", e)))?;
            ...
            let confidence = if model_ref.hasattr("predict_proba").unwrap_or(false) {
                match model_ref.call_method1("predict_proba", (&feature_array,)) {
                    Ok(proba) => {
                        ...
                        let probs: Vec<f64> = (0..proba_array.len()).map(|i| {
                            proba_array.get_item(i).ok().and_then(|item| item.extract().ok()).unwrap_or(0.5)
                        }).collect();
    ...
    fn predict_batch(&self, features: &[PyFeatureVector]) -> Result<Vec<PyPrediction>, PythonModelError> {
        ...
        Python::with_gil(|py| {
            ...
            let predictions = model_ref.call_method1("predict", (&feature_matrix,))
                .map_err(|e| PythonModelError::PredictionError(format!("Batch prediction failed: {}", e)))?;
            
            // Extract prediction values
            let pred_array = predictions.downcast::<PyList>()
                .map_err(|e| PythonModelError::PredictionError(format!("Failed to extract prediction array: {}", e)))?;
            ...
            for i in 0..pred_array.len() {
                if let Ok(item) = pred_array.get_item(i) {
                    let signal: f64 = item.extract()
                        .map_err(|e| PythonModelError::PredictionError(
                            format!("Failed to convert prediction {} to f64: {}", i, e)
                        ))?;
    ...
    fn is_ready(&self) -> bool {
        Python::with_gil(|py| {
            self.model.bind(py).hasattr("predict").unwrap_or(false)
        })
    }
}
...
mod tests {
    ...
    fn test_model_metadata() {
        ...
        assert_eq!(metadata.get("model_type"), Some(&"RandomForest".to_string()));
    ...
    fn test_prediction_struct() {
        ...
        assert_eq!(prediction_with_meta.metadata.get("model_version"), Some(&1.0));
    }
}