...
fn main() {
    ...
    println!("Zero-copy FileReader: {:.0} events/s ({:.1}x improvement)", 
            zerocopy_throughput, zerocopy_throughput / original_throughput);
...
fn test_original_reader(file_path: &PathBuf) -> f64 {
    ...
    println!("  Read {} events in {:.2}s", event_count, processing_time.as_secs_f64());
...
fn test_full_backtest(file_path: &PathBuf) -> f64 {
    ...
    println!("  Processed {} events in {:.2}s", event_count, processing_time.as_secs_f64());
    println!("  Total time with setup: {:.2}s", total_time.as_secs_f64());
    println!("  Trades executed: {}", report.performance_metrics.total_trades);
