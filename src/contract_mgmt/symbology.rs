...
use databento::dbn::{Metadata, SymbolMapping};
    ...
    pub fn add_mapping(&mut self, mapping: &SymbolMapping) {
        for interval in &mapping.intervals {
            if let Ok(inst_id) = interval.symbol.parse::<u32>() {
                ...
                self.id_to_symbol
                    .insert(inst_id, mapping.raw_symbol.clone());
            }
        }
    }
...
mod tests {
    ...
    fn test_symbology_manager() {
        ...
        manager.symbol_to_id.insert(
            "MESM5".to_string(),
            vec![(date!(2025 - 04 - 01), date!(2025 - 06 - 30), 1001)],
        );
        manager.symbol_to_id.insert(
            "MESU5".to_string(),
            vec![(date!(2025 - 07 - 01), date!(2025 - 09 - 30), 1002)],
        );
        ...
        assert_eq!(
            manager.get_instrument_id("MESM5", date!(2025 - 05 - 15)),
            Some(1001)
        );
        assert_eq!(
            manager.get_instrument_id("MESM5", date!(2025 - 07 - 01)),
            None
        );
        assert_eq!(
            manager.get_instrument_id("MESU5", date!(2025 - 08 - 01)),
            Some(1002)
        );
    ...
    fn test_multiple_intervals() {
        ...
        assert_eq!(
            manager.get_instrument_id("TEST", date!(2025 - 01 - 15)),
            Some(1001)
        );
        assert_eq!(
            manager.get_instrument_id("TEST", date!(2025 - 02 - 15)),
            Some(1002)
        );
        assert_eq!(
            manager.get_instrument_id("TEST", date!(2025 - 03 - 01)),
            None
        );
