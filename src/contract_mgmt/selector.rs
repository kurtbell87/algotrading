use crate::contract_mgmt::calendar::{
    ContractMonth, get_front_month, get_roll_date, is_in_roll_period,
};
...
impl ContractSelector {
    ...
    pub fn add_contract(&mut self, contract: FuturesContract) {
        self.contracts_by_id
            .insert(contract.instrument_id, contract.clone());
    ...
    pub fn get_next_contract(
        &self,
        current: &FuturesContract,
        date: Date,
    ) -> Option<&FuturesContract> {
...
mod tests {
    ...
    fn test_contract_selector() {
        ...
        let active = selector
            .get_active_contract("MES", date!(2025 - 05 - 01))
            .unwrap();
        assert_eq!(active.month, ContractMonth::June);

        let active = selector
            .get_active_contract("MES", date!(2025 - 06 - 12))
            .unwrap();
        ...
        let next = selector
            .get_next_contract(june, date!(2025 - 06 - 12))
            .unwrap();
        assert_eq!(next.month, ContractMonth::September);

        // Test contract chain
        let chain =
            selector.get_contract_chain("MES", date!(2025 - 05 - 01), date!(2025 - 10 - 01));
    ...
    fn test_load_from_symbology() {
        use databento::dbn::{MappingInterval, SymbolMapping};
