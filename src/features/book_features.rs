...
impl BookFeatures {
        ...
        match event {
            OrderBookEvent::OrderAdded { .. } => {
                ...
                if let OrderBookEvent::OrderAdded { order_id, .. } = event {
                    self.order_birth_events
                        .insert(*order_id, self.event_counter);
                }
            }
    ...
    pub fn add_to_vector(&self, features: &mut FeatureVector) {
        ...
        features.add(
            ...
            self.imbalance.volume_weighted_imbalance,
        );
        ...
        features.add(
            ...
            self.queue_metrics.modification_rate,
        );
