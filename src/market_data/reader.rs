...
use crate::core::types::{
    BookUpdate, BookUpdateType, InstrumentId, MarketUpdate, OrderId, Price, Quantity, Side, Trade,
};
...
pub type FileReader = ZeroCopyFileReader;
    ...
    pub fn update(&mut self, update: &MarketUpdate) {
        match update {
            MarketUpdate::Trade(trade) => {
                ...
                self.last_updates
                    .insert(trade.instrument_id, trade.timestamp);
            }
            MarketUpdate::OrderBook(book_update) => {
                self.last_updates
                    .insert(book_update.instrument_id, book_update.timestamp);
