...
use crate::strategy::{Strategy, StrategyConfig, StrategyContext, StrategyOutput, StrategyError, OrderSide};
...
impl TrendFollowingStrategy {
    ...
    fn update_momentum(&mut self) {
        ...
        if len >= self.tf_config.momentum_period {
            ...
            let past_price = self.price_history[len - self.tf_config.momentum_period].1.as_f64();
    ...
    /// Create entry order
    fn create_entry_order(&mut self, side: OrderSide, price: Price, context: &StrategyContext) -> OrderRequest {
    ...
    fn on_market_event(&mut self, event: &MarketEvent, context: &StrategyContext) -> StrategyOutput {
        ...
        if let Some((price, volume)) = price_volume {
            ...
            if self.check_exit_conditions(price, position) {
                output.orders.push(self.create_exit_order(position, context));
            }
            // Check entry conditions
                ...
                if self.momentum_confirms(new_trend) && self.volume_confirms() {
                    ...
                    output.orders.push(self.create_entry_order(side, price, context));
                }
            }
...
use crate::strategy::output::OrderType;
    ...
    fn test_moving_average_calculation() {
        ...
        for (i, price) in prices.iter().enumerate() {
            strategy.price_history.push((i as u64 * 1000, Price::from(*price as i64)));
        }
        ...
        assert!((strategy.short_ma.unwrap() - expected_short_ma).abs() < 0.1, 
               "Short MA {} should be close to expected {}", strategy.short_ma.unwrap(), expected_short_ma);
        ...
        assert!((strategy.long_ma.unwrap() - expected_long_ma).abs() < 0.1,
               "Long MA {} should be close to expected {}", strategy.long_ma.unwrap(), expected_long_ma);
        ...
        for (i, price) in prices2.iter().enumerate() {
            strategy2.price_history.push((i as u64 * 1000, Price::from(*price as i64)));
        }
    ...
    fn test_momentum_calculation() {
        ...
        assert!((momentum - expected_momentum).abs() < 0.001, 
               "Momentum {} should be close to expected {}", momentum, expected_momentum);
    ...
    fn test_order_generation_scenarios() {
        ...
        if new_trend == TrendDirection::Bullish && strategy.momentum_confirms(new_trend) {
            let order = strategy.create_entry_order(
                OrderSide::Buy,
                Price::from(105i64),
                &context,
            );
    ...
    fn test_order_creation() {
        ...
        let entry_order = strategy.create_entry_order(
            OrderSide::Buy,
            Price::from(10000i64),
            &context,
        );
